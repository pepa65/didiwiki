...
void wiki_print_data_as_html(HttpResponse *res, char *raw_page_data)
{
	...
	char *q = NULL, *link = NULL, *line = NULL;
	int line_len, i, j, skip_chars;
...
#define NUM_LIST_TYPES 2
	...
	// p accumalates non marked up text,
	// q is just a pointer to the end of the current line
	q = p;
	while ((line = get_line_from_string(&q, &line_len)))
	{
		...
		if (skip_to_content) goto line_content; // skip parsing any more initial chars
		// Tables
		if (*line == '|')
		{
			if (table_on==0)
...
line_content:
	...
	for (i=0; i<listtypes[ULIST].depth; i++) http_response_printf(res, "</ul>\n");
